        ...
        <main className="pt-32 pb-16 relative overflow-hidden">
          ...
          <div className="container mx-auto px-6 max-w-7xl relative z-10 animate-pulse">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
              ...
              <div className="lg:col-span-6 bg-white border border-slate-100 p-8 md:p-12 space-y-8">
      ...
      <main className="pt-32 pb-16 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 max-w-7xl relative z-10">
          ...
          <div className="mb-16 text-center lg:text-left">
            <span className="text-[10px] font-bold text-brand-blue uppercase tracking-[0.2em] block mb-3">{content.info.overline}</span>
            <h1 className="text-4xl md:text-6xl font-bold tracking-tighter text-brand-blue leading-none">
              {content.info.title}
            </h1>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
            {/* Contact Details & Form (Left - 5 columns) */}
            <div className="lg:col-span-5 space-y-8">
              {/* Spacious & Large Details Blocks */}
              <motion.div {...fadeInUp} className="space-y-6">
                {/* Address */}
                <div className="flex gap-4 items-start">
                  <div className="w-10 h-10 bg-slate-50 flex items-center justify-center flex-shrink-0 border border-slate-100/80">
                    <MapPin className="w-4.5 h-4.5 text-brand-blue" />
                  </div>
                  <div className="space-y-0.5">
                    <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Наш адрес</h3>
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div className="space-y-0.5">
                    <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Телефоны</h3>
                    {content.info.phones.map((phone, i) => (
                      <p key={i} className="text-[13px] font-bold text-slate-800 tracking-tight leading-normal">{phone}</p>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div className="space-y-0.5">
                    <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Часы работы</h3>
                    <p className="text-[13px] text-slate-700 leading-relaxed font-medium">{content.info.workHours.weekdays}</p>
                    <p className="text-[13px] text-slate-700 leading-relaxed font-medium">{content.info.workHours.sunday}</p>
                  </div>
                </div>
              </motion.div>

              {/* Refined Form stacked underneath */}
              <motion.div {...fadeInUp} className="pt-8 border-t border-slate-200/60 space-y-5">
                <h3 className="text-[11px] font-black uppercase tracking-[0.15em] text-brand-blue">{content.form.title}</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="space-y-0.5">
                    <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Имя</label>
                    <input 
                      type="text" 
                      required
                      ...
                      className="w-full bg-white border border-slate-200 px-3.5 py-2.5 text-xs focus:outline-none focus:border-brand-blue focus:ring-1 focus:ring-brand-blue/20 transition-all placeholder:text-slate-300"
                    />
                  </div>
                  <div className="space-y-0.5">
                  ...
                  <div className="space-y-0.5">
                    <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Сообщение</label>
                    <textarea 
                      required
                      ...
                      className="w-full bg-white border border-slate-200 px-3.5 py-2.5 text-xs focus:outline-none focus:border-brand-blue focus:ring-1 focus:ring-brand-blue/20 transition-all placeholder:text-slate-300 resize-none"
                    ></textarea>
                  </div>
                  <Button 
                    type="submit"
                    disabled={submitting}
                    className="w-full h-11 rounded-none uppercase text-[10px] tracking-[0.2em] font-bold mt-1"
                  >
            ...
            {/* Map (Right - 7 columns) */}
            <div className="lg:col-span-7 h-[450px] lg:h-[650px] relative">
              <motion.div 
              ...
              >
                <iframe 
                  ...
                  width="100%" 
                  height="100%" 
                  style={{ border: 0, filter: 'grayscale(1) contrast(1.15) invert(0)' }} 
                  allowFullScreen={true} 
                  loading="lazy" 
                  referrerPolicy="no-referrer-when-downgrade"
                ></iframe>
                
                <div className="absolute top-6 left-6 p-5 bg-white shadow-2xl border border-slate-100 max-w-[240px] space-y-3">
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-brand-blue rounded-full animate-pulse" />
                    <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-brand-blue">Showroom Liberty</span>
                  </div>
